...
## The following code was taken from the Gil-Mor/iFish repository linked below to
## remove the fisheye effect from our images.
...
def get_fish_xn_yn(source_x, source_y, radius, distortion):
    ...
    return source_x / (1 - (distortion * (radius**2))), source_y / (
        1 - (distortion * (radius**2))
    )
...
def parse_args(args=sys.argv[1:]):
    ...
    parser = argparse.ArgumentParser(
        description="Apply fish-eye effect to images.", prog="python3 fish.py"
    )
    ...
    parser.add_argument(
        "-o",
        "--outpath",
        help="file path to write output to." " format: <path>.<format(jpg,png,etc..)>",
        type=str,
        default="fish.png",
    )
...
if __name__ == "__main__":
    ...
    if os.path.exists(args.outpath):
        ans = input(args.outpath + " exists. File will be overridden. Continue? y/n: ")
        if ans.lower() != "y":
            print("exiting")
            sys.exit(0)
